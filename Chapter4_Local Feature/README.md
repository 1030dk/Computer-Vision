...
- SIFT: 이미지의 크기(Scale)가 커지거나 작아져도, 혹은 이미지가 회전하거나 조명이 바뀌어도 변하지 않는 강력한 특징점을 추출하는 알고리즘.
- 특징점: 이미지 내에서 코너나 모서리처럼 주변과 뚜렷하게 구별되어 추적하기 쉬운 좌표(x, y)
- 디스크립터: 추출된 특징점 주변의 픽셀들이 어떤 방향과 크기로 변화하는지를 계산하여 숫자로 나타냄.
